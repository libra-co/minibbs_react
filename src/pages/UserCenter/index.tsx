    ...
    return (
        <div className='user-center-page' >
            ...
            <Space className='block-body' direction='vertical' style={{ '--gap-vertical': '1px' }}>
                <div>
                    <a className='link-btn cancel-text-margin-right'>信箱</a>(0/0)
                    <a className='link-btn'>好友</a>({user?.friendsNum})
                </div>
                <p>我的ID:{user.uid}</p>
                <p>我的妖精:{user.coin}</p>
                <p>
                    <a className='link-btn cancel-text-margin-right' >充值妖精</a>
                    /
                    <a className='link-btn'>转账</a>
                    /
                    <a className='link-btn'>明细</a>
                </p>
                <p>银行账户: <span style={{ color: 'red' }}>银行模块未实现</span><a className='link-btn' >取款</a></p>
                ...
                <p>有效期至:<a className='link-btn' >{user.expireTime || '无限期'}</a></p>
                <p>管理权限:{RoleEnum[user.role]}</p>
                <p><a className='link-btn cancel-text-margin-right'>我的勋章</a>:</p>
                <p><a className='link-btn cancel-text-margin-right'>申请勋章</a>/<a className='link-btn' >购买勋章</a></p>
            </Space>
